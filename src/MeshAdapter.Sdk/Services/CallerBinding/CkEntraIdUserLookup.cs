...
/// <summary>
///     Tenant-repository-backed <see cref="IEntraIdUserLookup" /> (AB#5124). Reads the AB#5122
///     verified-identifier directory (<c>System.Identity/VerifiedExternalIdentifier</c>) with the
///     SAME generic CK entity access the adapter already uses for
///     <c>System.Communication/ServiceAccountConfiguration</c> (see
///     <c>ServiceAccountTokenService.ReadConfigurationAsync</c>) — the identity CK model lives in the
///     tenant's own runtime database, so no service-to-service call to the identity service is
///     needed. The adapter SDK cannot reference the generated identity CK model, so the type ids,
///     attribute names and association roles are addressed by their (non-versioned) full names,
///     exactly as the service-account read does.
/// </summary>
internal sealed class CkEntraIdUserLookup(
    ISystemContext systemContext,
    ICkCacheService ckCacheService,
    ILogger<CkEntraIdUserLookup> logger) : IEntraIdUserLookup
{
    ...
    // Numeric key of RtIdentifierKindEnum.EntraIdObjectId in System.Identity (kept identical to
    // ChannelIdentifierKind.EntraIdObjectId — the AB#5122/5126 enums mirror each other by value).
    private const int EntraIdObjectIdKind = (int)ChannelIdentifierKind.EntraIdObjectId;

    public async Task<EntraIdCallerRecord?> FindByObjectIdAsync(string tenantId, string objectId,
        CancellationToken cancellationToken = default)
    {
        var tenantRepository = await systemContext.TryFindTenantRepositoryAsync(tenantId);
        if (tenantRepository == null)
        {
            logger.LogDebug("[{TenantId}] No tenant repository; cannot resolve EntraID object id", tenantId);
            return null;
        }
        ...
        try
        {
            ...
            var binding = await FindBindingAsync(tenantRepository, session, objectId);
            if (binding == null)
            {
            ...
            if (userEntity == null)
            {
                await session.CommitTransactionAsync();
                logger.LogWarning(
                    "[{TenantId}] VerifiedExternalIdentifier '{BindingRtId}' (EntraID) has no bound user; treating as unresolved",
            ...
            return new EntraIdCallerRecord(
        ...
        {
            // The tenant has no System.Identity model imported (no EntraID IdP): unresolved, not an
            // error — the binder applies the trigger's anonymous-mode decision.
            logger.LogDebug(ex,
                "[{TenantId}] System.Identity model not available; EntraID object id is unresolved", tenantId);
            return null;
        }
    }

    /// <summary>
    ///     Finds the single <c>VerifiedExternalIdentifier</c> for the oid. Filters on the highly
    ///     selective <c>IdentifierValue</c> and disambiguates the kind in memory — the (kind, value)
    ///     pair is unique per tenant (AB#5122 Unique index), so at most one row survives.
    /// </summary>
    private static async Task<RtEntity?> FindBindingAsync(ITenantRepository tenantRepository,
        IOctoSession session, string objectId)
    {
        var queryOptions = RtEntityQueryOptions.Create();
        queryOptions.AddFieldFilter("IdentifierValue", FieldFilterOperator.Equals, objectId);
        ...
        return result.Items.FirstOrDefault(e =>
            e.GetAttributeValueOrDefault<int>("IdentifierKind") == EntraIdObjectIdKind);
    }
    ...
    ///     enums share their numeric keys one-for-one (None=0, Weak=1, Strong=2), documented on both
    ///     sides. An out-of-range value is clamped to <see cref="CallerTrustLevel.None" /> fail-closed.
    /// </summary>
