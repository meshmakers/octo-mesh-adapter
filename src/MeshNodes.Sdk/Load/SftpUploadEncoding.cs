...
public static class SftpUploadEncoding
{
    ...
    public static Encoding Resolve(string? encodingName)
    {
        ...
        {
            throw new ArgumentException(
                $"Encoding '{encodingName}' is not supported: the SFTP nodes neither write nor consume a byte-order mark, so a multi-byte encoding with byte-order semantics would be ambiguous in both directions. Use utf-8 or a single-byte code page such as windows-1252.",
                nameof(encodingName));
        }
