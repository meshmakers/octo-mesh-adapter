...
public record LlmQueryNodeConfiguration : SourceTargetPathNodeConfiguration
{
    ...
    /// (e.g. http://localhost:11434/v1/). Null = OpenAI cloud. For Anthropic it overrides
    /// the default https://api.anthropic.com endpoint (proxies, gateways); null = public API.
    /// </summary>
    ...
    /// <summary>
    /// Sampling temperature (0.0-1.0); lower is more deterministic. Null = 0.3 unless TopP
    /// is set (the node applies the default only when neither control is configured).
    /// Mutually exclusive with TopP.
    /// </summary>
