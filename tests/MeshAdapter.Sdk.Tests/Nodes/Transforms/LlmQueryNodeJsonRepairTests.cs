...
public class LlmQueryNodeJsonRepairTests
{
    ...
    [Fact]
    public void ExtractJsonFromText_ProseBracketsBeforeTheObject_ReturnsTheFirstValidCandidate()
    {
        var extracted = LlmJsonExtractor.ExtractJsonFromText("""Result [draft]: {"a":1} — done.""");

        Assert.Equal("""{"a":1}""", extracted);
    }

    [Fact]
    public void ExtractJsonFromText_NoCandidateParses_ReturnsTheFirstForMechanicalRepair()
    {
        var extracted = LlmJsonExtractor.ExtractJsonFromText("""Result: {"a":1,} then {"b":2,}""");

        // Both are broken; the first is handed to the repair tier, matching the previous contract.
        Assert.Equal("""{"a":1,}""", extracted);
    }
}
