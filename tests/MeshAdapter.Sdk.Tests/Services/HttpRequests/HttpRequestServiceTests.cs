...
public class HttpRequestServiceTests
{
    ...
    [Fact]
    public async Task SendRequestAsync_JsonBodyWithQuotedCharsetParameter_DecodedWithTheDeclaredCharset()
    {
        JsonNode? receivedInput = null;
        var options = CreateRouteOptions("/api/json", HttpMethod.Post, input =>
        {
        ...
        // RFC 9110 allows the parameter value as a quoted-string; the quotes are not part of the name.
        var bytes = Encoding.Unicode.GetBytes("{\"name\":\"Grüße\"}");
        var context = CreateHttpContext("POST", $"/{TenantId}/api/json", bytes, "application/json; charset=\"utf-16\"");
        var result = await _service.SendRequestAsync(context);
    ...
    [Fact]
    public async Task SendRequestAsync_CsvBodyDeclaredLatin1_DecodedWithTheDeclaredCharset()
    {
        JsonNode? receivedInput = null;
        var options = CreateRouteOptions("/api/csv", HttpMethod.Post, input =>
        {
            receivedInput = input;
            return Task.FromResult<JsonNode?>(null);
        });
        ...
        // Not text/plain: every text media type has to go through the charset-aware reader.
        var bytes = Encoding.Latin1.GetBytes("name;city\nGrüße;Köln");
        var context = CreateHttpContext("POST", $"/{TenantId}/api/csv", bytes, "text/csv; charset=iso-8859-1");
        var result = await _service.SendRequestAsync(context);

        Assert.True(result);
        Assert.Equal("name;city\nGrüße;Köln", receivedInput!["body"]?.ToString());
    }
